...
def seed(db:Session):
    ...
    electronic_cards=[
        KnowledgeCard(tenant_id=tenant.id,workspace_id=ws.id,business_concept_id=supplier_concept.id,title="Electronic Manufacturer Quality Certification",summary="ISO 9001 certification, audit status, and quality-system controls required for electronics suppliers.",body="Electronic manufacturers must maintain a current ISO 9001 certificate, documented internal audits, management review, calibration controls, and corrective action processes.",knowledge_type="qualification_evidence",lifecycle_status="published",approval_status="approved",authority_level="quality_standard",classification_rank=20,ai_usage_allowed=True,trust_score=.92,owner_id=user.id,approved_by=user.id,approved_at=utcnow()),
        KnowledgeCard(tenant_id=tenant.id,workspace_id=ws.id,business_concept_id=supplier_concept.id,title="Component Traceability and Counterfeit Prevention",summary="Lot traceability, authorized sourcing, incoming inspection, and counterfeit component controls.",body="Suppliers must provide lot and date-code traceability, source components from authorized channels, retain certificates of conformance, and maintain counterfeit detection and escalation procedures.",knowledge_type="qualification_evidence",lifecycle_status="published",approval_status="approved",authority_level="supplier_control",classification_rank=20,ai_usage_allowed=True,trust_score=.95,owner_id=user.id,approved_by=user.id,approved_at=utcnow()),
        KnowledgeCard(tenant_id=tenant.id,workspace_id=ws.id,business_concept_id=supplier_concept.id,title="Electronics Manufacturing Capability Review",summary="SMT, PCB assembly, test capability, ESD control, process validation, and change notification.",body="The capability review covers SMT and PCB assembly equipment, automated optical inspection, functional testing, ESD controls, workmanship standards, process validation, maintenance, and customer change notification.",knowledge_type="qualification_evidence",lifecycle_status="published",approval_status="approved",authority_level="technical_assessment",classification_rank=20,ai_usage_allowed=True,trust_score=.91,owner_id=user.id,approved_by=user.id,approved_at=utcnow())
    ]
