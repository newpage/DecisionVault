...
class DecisionMemoryService:
    ...
    def list_precedents(
        ...
        *,
        ...
        current = self._require_decision(
        ...
        for historical in candidates:
            historical_profile = self._profile(
                historical, clearance_rank, role_ids, permissions
            )
            if (
                outcome_classification
                and historical_profile.effectiveness_classification
                != outcome_classification
            ):
                continue
            ...
            if (
                ...
                < RELEVANCE_THRESHOLDS[minimum_relevance]
            ):
                continue
            results.append(
                self._result(
        ...
        results.sort(
            key=lambda item: (
                ...
                -item.historical_decision.created_at.timestamp(),
                item.historical_decision.id,
            )
        )
    ...
    def compare(
        ...
        *,
        ...
        historical = self._repository.get_historical_decision(
            tenant_id=tenant_id,
            current_decision_id=decision_id,
        ...
        comparison = compare_profiles(
            self._profile(current, clearance_rank, role_ids, permissions),
            self._profile(historical, clearance_rank, role_ids, permissions),
        )
        summary = self._summary(
            tenant_id, historical, clearance_rank, role_ids, permissions
        )
        governance = None
        if REVIEW_VIEW_PERMISSION in permissions:
            governance = {
                "review_types": sorted(
                    {
                        item.review_type
                        for item in self._repository.reviews(
                ...
                "approval_actions": [
                    item.action
                    for item in self._repository.approvals(
                        tenant_id=tenant_id, decision_id=historical.id
                    )
                ],
                "conditions": [
                    {"status": item.status, "text": item.condition_text}
                    for item in self._repository.conditions(
                        tenant_id=tenant_id, decision_id=historical.id
                    )
                ],
                "material_findings": [
                    ...
                    for item in self._repository.findings(
                        tenant_id=tenant_id, decision_id=historical.id
                    )
        ...
        if OUTCOME_VIEW_PERMISSION in permissions:
            assessment = self._repository.latest_assessment(
                tenant_id=tenant_id, decision_id=historical.id
            )
            outcome = {
                ...
                "outcomes": [
                    ...
                    for item in self._repository.outcomes(
                        tenant_id=tenant_id, decision_id=historical.id
                    )
                ],
            }
            lessons = [
                ...
                for item in self._repository.lessons(
                    tenant_id=tenant_id, decision_id=historical.id
                )
            ]
        return DecisionComparisonResponse(
            current_decision={
                ...
                "business_concept_id": current.business_concept_id,
            },
    ...
    def _summary(self, tenant_id, historical, clearance_rank, role_ids, permissions):
        evidence = (
            self._repository.authorized_evidence(
                tenant_id=tenant_id,
                decision_id=historical.id,
                clearance_rank=clearance_rank,
                role_ids=role_ids,
            )
            if EVIDENCE_VIEW_PERMISSION in permissions
            else None
        )
        approvals = (
            self._repository.approvals(tenant_id=tenant_id, decision_id=historical.id)
            if REVIEW_VIEW_PERMISSION in permissions
            else None
        )
        conditions = (
            self._repository.conditions(tenant_id=tenant_id, decision_id=historical.id)
            if REVIEW_VIEW_PERMISSION in permissions
            else None
        )
        findings = (
            self._repository.findings(tenant_id=tenant_id, decision_id=historical.id)
            if REVIEW_VIEW_PERMISSION in permissions
            else None
        )
        assessment = (
            self._repository.latest_assessment(
                tenant_id=tenant_id, decision_id=historical.id
            )
            if OUTCOME_VIEW_PERMISSION in permissions
            else None
        ...
        lessons = (
            self._repository.lessons(tenant_id=tenant_id, decision_id=historical.id)
            if OUTCOME_VIEW_PERMISSION in permissions
            else None
        )
        return HistoricalDecisionSummary(
            ...
            business_concept_name=self._repository.concept_name(
            ...
            evidence_types=sorted({item.snapshot_knowledge_type for item in evidence})
            if evidence is not None
            else None,
            material_conditions=[
                item.condition_text for item in conditions if item.status == "open"
            ]
            if conditions is not None
            else None,
            material_findings=[
                item.title for item in findings if item.severity in {"high", "critical"}
            ]
            if findings is not None
            else None,
            lessons=[item.description for item in lessons]
